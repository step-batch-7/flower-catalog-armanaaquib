...
const getCommentsDetail = function () {
  if (doesNotFileExist(STORAGE_FILE)) {
    return [];
  }
...
const serveGuestBookPage = function (req, res) {
  const commentsDetail = getCommentsDetail();
  let comments = Comments.load(commentsDetail);
  comments = comments.toHtml();
...
const addComment = function (data) {
  const commentDetails = getCommentsDetail();
  const comments = Comments.load(commentDetails);
  ...
  fs.writeFileSync(STORAGE_FILE, comments.toJSON(), 'utf8');
};
